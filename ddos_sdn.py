...
def calculate_acc_model(model, Xtest_data, ytest_data):
    predicted = model.predict(Xtest_data)
    return metrics.roc_auc_score(ytest_data, predicted,  average='micro')
    #return model.score(Xtest_data, ytest_data)
...
for round in range(rounds):
    ...
    models=["BaggingClassifier", "MLP", "KNN", "RandomForestClassifier", "DecisionTreeClassifier", "GradientBoostingClassifier", "AdaBoostClassifier"]
    for i in range(nodes):
        ...
        print(printScore(predicted, y_test, model_name)) ############################
...
#print("devices ",devices)
    avg[0] = avg[0]/nodes; avg[1] = avg[1]/nodes
...
#Pick random device to be a Miner
    ...
    m_name =devices[miner_id][3]
    if m_name in miner_models: 
       miner_models[m_name] +=1#.append(devices[miner_id][3])
    else:
       miner_models[m_name]=1
       #miner_models[miner_id]#.append(devices[miner_id][3])
#miner_model = miner_model[0]
    ...
    for device in devices:
        if(miner_id != device):
            ...
            device_accuracy= device_model.score(miner_Xtest_data, miner_ytest_data)
    ...
    plt.title('Receiver Operating Characteristic for round '+str(round))
    plt.legend(loc="lower right")
    plt.savefig("auc_round_"+str(round)+".jpg")
    #plt.show()
    ...
    if round <= round_window:
        ...
        for device in devices:
            ...
            device_Xtest_data = devices[device][1]
            device_ytest_data = devices[device][2]
            device_accuracy= devices[device][0].score(miner_Xtest_data, miner_ytest_data)
        #devices_acc[device] = device_accuracy
            print("device_accuracy after assiging best model", device, device_accuracy)
...
for i in devices:
    # print(bclf[i].score(X_train,y_train))
    if devices[i][0].score(X_train, y_train) > m1:
        m1 = devices[i][0].score(X_train, y_train)
        print("Best device in training ", i)
    sum1 += devices[i][0].score(X_train, y_train)

    if devices[i][0].score(X_test, y_test) > m2:
        m2 = devices[i][0].score(X_test, y_test)
        print("Best device in testing ", i)
    sum2 += devices[i][0].score(X_test, y_test)
print('Best train:',m1)
print('Best test:',m2)
print('Avg train:',sum1/nodes)
print('Avg test:',sum2/nodes)
print("Miner models -->", miner_models) 
